...
//### Compatibility types...
private protocol AVCaptureDeviceDiscoverySessionType: AnyObject {
    @available(iOS 10.0, *)
...
//    @objc dynamic var stillImageOutput: AVCapturePhotoOutput? //### iOS >= 10.0
    ...
    private var backgroundRecordingID: UIBackgroundTaskIdentifier = UIBackgroundTaskIdentifier.invalid
    ...
    private func configureManualHUD() {
        ...
        if let videoDevice = self.videoDevice {//###
            self.focusModeControl.selectedSegmentIndex = self.focusModes.firstIndex(of: videoDevice.focusMode)!
            for mode in self.focusModes {
                self.focusModeControl.setEnabled(videoDevice.isFocusModeSupported(mode), forSegmentAt: self.focusModes.firstIndex(of: mode)!)
            }
        }
                ...
                if let old = oldValue as? Int {
                    ...
                    if oldMode != newMode && oldMode == .custom {
                        do {
                            ...
                            self.videoDevice!.activeVideoMaxFrameDuration = CMTime.invalid
                            self.videoDevice!.activeVideoMinFrameDuration = CMTime.invalid
                        } catch let error {
                            NSLog("Could not lock device for configuration: \(error)")
                        }
                    }
                ...
                if #available(iOS 9.0, *) {
                    ...
                    self.stillImageOutput!.captureStillImageBracketAsynchronously(from: self.stillImageOutput!.connection(with: AVMediaType.video)!,
                        ...
                        if error != nil {
                            ...
                            NSLog("Lens Stabilization State: \(CMGetAttachment(imageDataSampleBuffer!, key: kCMSampleBufferAttachmentKey_StillImageLensStabilizationInfo, attachmentModeOut: nil)!)")
                            let imageData = AVCaptureStillImageOutput.jpegStillImageNSDataRepresentation(imageDataSampleBuffer!)
